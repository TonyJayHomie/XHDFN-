// cocodem-replacement.worker.js -- CFC12
// Drop-in Cloudflare Worker that emulates cocodem's openclaude.111724.xyz
// + cfc.aroic.workers.dev contract. Zero phone-home.
//
// CFC12 changes vs CFC11:
//   * /oauth/authorize now redirects to the worker's OWN /oauth/redirect page
//     (which postMessages the extension), instead of 302'ing directly to
//     chrome-extension://... -- Chrome blocks cross-origin redirects to
//     chrome-extension URLs (ERR_BLOCKED_BY_CLIENT in your screenshot).
//   * /oauth/redirect page now also tries chrome.runtime.connect AND
//     window.opener.postMessage, and has a manual "Open side panel" button
//     so the flow can still be completed if scripts are blocked.
//   * Proxy token is now shaped as a real JWT (header.payload.signature, all
//     base64url) so the extension's internal token-shape validators accept it.
//   * /cfc-debug-ping echoes back severed:true + cfcVersion:"CFC12".
//   * Everything else identical to CFC11.
...
// fires from a value flip (React #185 trap).
...
// ── CFC12: JWT-shape token ─────────────────────────────────────────────────
// "cfc-local-permanent.cfc-local-permanent.cfc-local-permanent" from CFC11
// was NOT a valid JWT (the middle segment base64url-decoded to garbage and
// the extension's validator throws "Invalid bearer token"). Build a real
// three-segment JWT with base64url-encoded JSON header + payload.
function b64url(s) {
  let b;
  if (typeof s === "string") {
    b = btoa(unescape(encodeURIComponent(s)));
  } else {
    b = btoa(unescape(encodeURIComponent(JSON.stringify(s))));
  }
  return b.replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}
const TOKEN_HEADER  = b64url({ alg: "none", typ: "JWT" });
const TOKEN_PAYLOAD_B = b64url({
  iss: "cfc-local",
  sub: ACCOUNT_UUID,
...
const TOKEN_SIG     = b64url("cfc-local-permanent-signature");
const TOKEN_STATIC  = `${TOKEN_HEADER}.${TOKEN_PAYLOAD_B}.${TOKEN_SIG}`;
...
function dbg(tag, ...args) {
  if (!DEBUG) return;
  try { console.log(`[CFC12/${tag}]`, ...args); } catch (e) {}
}
...
const ACCOUNT = {
  ...
  has_claude_pro: true, has_claude_max: true,
...
const ORGANIZATION = {
  ...
  active_subscription: {
    plan: "claude_max", status: "active", type: "claude_max",
...
const PROFILE = {
  ...ACCOUNT,
  account: ACCOUNT, organization: ORGANIZATION, organizations: [ORGANIZATION],
  memberships: [{ organization: ORGANIZATION, role: "admin",
                  joined_at: "2024-01-01T00:00:00Z" }],
  active_organization_uuid: ORG_UUID,
};
const ACCOUNT_SETTINGS = {
  uuid: ACCOUNT_UUID, enabled_mcp_tools: {},
...
<meta charset="utf-8"><title>Open Claude -- CFC12 Worker</title>
...
<div class="card">
<span class="tag">CFC12 -- SEVERED</span>
<h1>Open Claude</h1>
...
// /oauth/redirect: runs inside a normal https page, so it CAN call
// chrome.runtime.sendMessage to the extension and also window.opener.postMessage
// to complete the flow. Falls back to a manual button.
function oauthRedirectHtml(code, state) {
  return `<!doctype html><html><head><meta charset="utf-8">
<title>Claude in Chrome - Complete sign-in</title>
<style>body{font:15px system-ui;background:#f9f8f3;color:#3d3929;margin:0;
padding:48px 24px;text-align:center}.c{max-width:420px;margin:0 auto;
background:white;border:1px solid #e5e2d9;border-radius:24px;padding:32px}
h1{font:400 24px Georgia,serif;margin:0 0 10px}p{color:#6b6651;margin:0 0 16px}
button,a.btn{display:inline-block;background:#d97757;color:#fff;border:0;
padding:12px 24px;border-radius:10px;font-size:14px;font-weight:700;
cursor:pointer;text-decoration:none}code{background:#f4f1ea;padding:2px 6px;
border-radius:5px;font-size:12px}.tag{display:inline-block;background:#2d6a4f;
color:#fff;padding:2px 10px;border-radius:10px;font-size:11px;font-weight:700;
letter-spacing:.5px;margin-bottom:12px}</style></head>
<body><div class="c">
<div class="tag">CFC12</div>
<h1>Sign-in complete</h1>
<p>If the side panel did not open automatically, click below.</p>
<button id="go">Open Claude side panel</button>
<p style="margin-top:20px;font-size:12px;color:#999">
code=<code>${code}</code> state=<code>${state}</code></p>
</div>
<script>
(function(){
  ...
  function notify(){
    // 1. postMessage to opener (original cocodem flow)
    try { window.opener && window.opener.postMessage(
      {type:"oauth_redirect", code, state, redirect_uri:location.href}, "*"
    ); } catch(e){}
    // 2. chrome.runtime.sendMessage (externally_connectable path)
  ...
  notify();
  // Auto-close after 4s only if we still have an opener.
...
async function handle(request) {
  ...
  if (method === "OPTIONS") { dbg("preflight", path); return noContent(); }

  // Unwrap request.js proxy-wrapping: /https://host/path -> /path
  let p = path;
  ...
  // Debug ping (curl-friendly sentinel)
  if (bare === "/cfc-debug-ping") {
    const echo = {
      cfcVersion: "CFC12", severed: true,
      now: new Date().toISOString(), method, path, bare, origin: url.origin,
      headers: {
        Authorization: redact(inAuth), "x-api-key": redact(inXkey),
        "user-agent": (request.headers.get("User-Agent") || "").slice(0, 80),
      },
      token_shape: {
        len: TOKEN_STATIC.length,
        segments: TOKEN_STATIC.split(".").length,
        header_sample: TOKEN_HEADER.slice(0, 12) + "...",
      },
    };
  ...
  if (TELEMETRY.some(t => p.includes(t))) {
    dbg("tel.sink", p.slice(0, 120)); return noContent();
  }

  // Bootstrap features (sidepanel fix)
  ...
  // OAuth profile/account/settings/orgs
  ...
  // ── CFC12: /oauth/authorize redirects to OUR OWN /oauth/redirect page,
  //          not directly to chrome-extension:// (Chrome blocks that cross-
  //          origin top-level nav with ERR_BLOCKED_BY_CLIENT).
  if (bare.endsWith("/oauth/authorize")) {
    const sp    = url.searchParams;
    const code  = "cfc-local-permanent";
    const state = sp.get("state") || "cfc-local";
    const dest  = new URL(url.origin + "/oauth/redirect");
    dest.searchParams.set("code",  code);
    dest.searchParams.set("state", state);
    // Preserve the caller's redirect_uri so the redirect page can forward it.
    const orig = sp.get("redirect_uri");
    if (orig) dest.searchParams.set("redirect_uri", orig);
    dbg("oauth.authorize", "-> own /oauth/redirect");
    return Response.redirect(dest.toString(), 302);
  }

  if (bare.endsWith("/oauth/redirect")) {
    const sp    = url.searchParams;
    const code  = sp.get("code")  || "cfc-local-permanent";
    const state = sp.get("state") || "cfc-local";
    dbg("oauth.redirect.html", "code=" + code + " state=" + state);
  ...
  if (bare.endsWith("/api/web/url_hash_check/browser_extension")) {
    dbg("url_hash_check"); return json({ allowed: true });
  }

  // /api/options + /api/identity
  if (bare === "/api/options") {
    dbg("api.options");
    return json({
      cfcVersion: "CFC12", severed: true,
      mode: "", cfcBase: url.origin + "/", anthropicBaseUrl: "",
      apiBaseUrl: "", apiBaseIncludes: [],
      proxyIncludes: [
      ...
      apiKey: "", authToken: TOKEN_STATIC, blockAnalytics: true,
    });
  }
