...
def _build_options_response() -> dict:
    """Build /api/options -- full contract that getOptions() in request.js parses.
    Only intentional deviation from cocodem's live response:
    uiNodes is [] (cocodem's 3 nodes inject the credential-harvesting UI -- stripped).
    All identity / backend / auth fields MUST be present here -- getOptions() reads
    them directly and mirrors to localStorage + chrome.storage.local.
    Removing them breaks backend routing and auth entirely."""
    discard = list(TELEMETRY_DOMAINS) if IDENTITY.get("blockAnalytics", True) else []
    return {
