...
CFC11 surgical changes from CFC10 (which kept all CFC8/9 work intact):
  ...
  * Strengthened request.js options-page injector:
      ...
      - ADDS a 3-second fallback that floats a fixed-position visible button
        labelled "\u26a1 CFC SEVERED \u2014 Backend Settings" if no anchor found
      - ADDS a top-of-page "CFC SEVERED" badge so visual confirmation that the
        phone-cord is cut is immediate, regardless of which options layout
        cocodem is rendering
...
if (globalThis.window) {
  ...
  // ── CFC12: sidepanel dropdown injector ──────────────────────────────────────
  // The cocodem header dropdown (Convert to task / Settings / Language)
  // is rendered on sidepanel.html. We watch the DOM and, when that menu
  // mounts, append a "Backend Settings (CFC)" row + a fixed-position
  // floating button as a guaranteed fallback. Opens the Backend Settings
  // UI in a new tab via chrome.tabs.create(options.html#backendsettings).
  if (location.pathname == "/sidepanel.html") {
    console.log("[CFC12] sidepanel injector active")

    const openBackendSettings = () => {
      const url = chrome?.runtime?.getURL
        ? chrome.runtime.getURL("options.html#backendsettings")
        : "/options.html#backendsettings"
      try { chrome.tabs.create({ url }) } catch (e) {
        try { window.open(url, "_blank") } catch (e2) {}
      }
    }

    // Floating bottom-right button -- always appears within 2s, no DOM deps.
    const installSidepanelFloat = () => {
      if (document.getElementById("__cfc_sp_float")) return
      if (!document.body) return
      const btn = document.createElement("button")
      btn.id = "__cfc_sp_float"
      btn.textContent = "\u2699\ufe0f Backend Settings"
      btn.title = "CFC12 \u2014 Open local backend settings"
      btn.style.cssText =
        "position:fixed;bottom:14px;right:14px;z-index:2147483647;" +
        "background:#d97757;color:#fff;border:0;font:700 12px system-ui;" +
        "padding:8px 12px;border-radius:10px;cursor:pointer;" +
        "box-shadow:0 3px 12px rgba(0,0,0,.4);"
      btn.addEventListener("click", (e) => { e.preventDefault(); openBackendSettings() })
      document.body.appendChild(btn)
    }

    // Tiny "CFC" severed pill at top-right of the sidepanel (always visible).
    const installSidepanelPill = () => {
      if (document.getElementById("__cfc_sp_pill")) return
      if (!document.body) return
      const pill = document.createElement("div")
      pill.id = "__cfc_sp_pill"
      pill.textContent = "CFC"
      pill.title = "CFC12 \u2014 local proxy active"
      pill.style.cssText =
        "position:fixed;top:8px;left:8px;z-index:2147483647;" +
        "background:#2d6a4f;color:#fff;font:800 10px system-ui;" +
        "padding:2px 7px;border-radius:10px;letter-spacing:.5px;" +
        "pointer-events:none;opacity:.85;"
      document.body.appendChild(pill)
    }

    // Dropdown injector: watch for the cocodem header menu and add a row.
    // The menu contains plain-text items "Convert to task", "Settings",
    // "Language". We find any menu that contains the Settings item, then
    // prepend/append our own item styled like its siblings.
    const tryInjectDropdown = () => {
      if (document.getElementById("__cfc_sp_menuitem")) return true
      // Candidate menus: role="menu", or any element with the 3 known labels.
      const candidates = document.querySelectorAll(
        '[role="menu"], [role="menuitem"], [role="listbox"], ul, div'
      )
      for (const m of candidates) {
        ...
        // Find the Settings item element to clone its styling.
        ...
        // Walk up to the clickable row (usually a button or [role="menuitem"]).
        ...
        // Clone it so we inherit padding/typography/hover classes.
        ...
        // Add a small CFC tag so it's obviously ours.
        const tag = document.createElement("span")
        tag.textContent = "CFC"
        tag.style.cssText =
          "margin-left:8px;background:#2d6a4f;color:#fff;" +
          "font:700 9px system-ui;padding:1px 5px;border-radius:6px;"
        clone.appendChild(tag)
        // Insert right after the Settings row.
        if (row.nextSibling) row.parentElement.insertBefore(clone, row.nextSibling)
        else row.parentElement.appendChild(clone)
        console.log("[CFC12] injected Backend Settings into sidepanel dropdown")
        return true
      }
      return false
    }

    const _spObserver = new MutationObserver(() => {
      installSidepanelPill()
      tryInjectDropdown()
    })
    const bootSp = () => {
      installSidepanelPill()
      installSidepanelFloat()
      _spObserver.observe(document.body, { childList: true, subtree: true })
    }
    if (document.body) bootSp()
    else window.addEventListener("DOMContentLoaded", bootSp)
  }

  if (location.pathname == "/options.html") {
    // ── Hash router (works regardless of whether the inject button fires) ──
    const handleHash = () => {
    ...
    else window.addEventListener("DOMContentLoaded", handleHash)

    // ── "CFC SEVERED" badge -- always visible, top-right, fixed position ──
    // This is the visual confirmation the phone-cord has been cut. It does
    // not depend on cocodem's DOM at all.
    const installBadge = () => {
      if (document.getElementById("__cfc_severed_badge")) return
      if (!document.body) return
      const badge = document.createElement("div")
      badge.id = "__cfc_severed_badge"
      badge.textContent = "\u26a1 CFC SEVERED \u2014 local proxy active"
      badge.style.cssText =
        "position:fixed;top:10px;right:10px;z-index:2147483647;" +
        "background:#2d6a4f;color:#fff;font:700 12px system-ui;" +
        "padding:6px 12px;border-radius:14px;box-shadow:0 2px 8px rgba(0,0,0,.35);" +
        "pointer-events:none;letter-spacing:.3px;"
      document.body.appendChild(badge)
    }

    // ── Inline anchor: try to find "Log out" / "API Key" / settings header
    //    and inject a Backend Settings button next to it. ──
    const tryInlineInject = () => {
      if (document.getElementById("__cfc_backend_btn")) return true
      ...
      link.className = anchorEl.className
      link.innerHTML = "\u2699\ufe0f Backend Settings"
      link.style.color      = "#e07a5f"
      link.style.fontWeight = "700"
      link.style.marginRight = "8px"
      anchorEl.parentElement.insertBefore(link, anchorEl)
      return true
    }

    // ── Floating fallback button: GUARANTEED visible, regardless of DOM. ──
    const installFloatingBtn = () => {
      if (document.getElementById("__cfc_backend_btn_float")) return
      if (!document.body) return
      const btn = document.createElement("a")
      btn.id = "__cfc_backend_btn_float"
      btn.href = "/options.html#backendsettings"
      btn.textContent = "\u2699\ufe0f Backend Settings (CFC)"
      btn.style.cssText =
        "position:fixed;bottom:20px;right:20px;z-index:2147483647;" +
        "background:#d97757;color:#fff;font:700 14px system-ui;" +
    ...
    const _observer = new MutationObserver(() => {
      installBadge()
      if (tryInlineInject()) {
        // Don't disconnect -- React may rerender and remove our node;
        // re-inject if needed. But cap CPU by checking the id first.
      }
    })
    if (document.body) {
      installBadge()
    ...
    // 3-second hard guarantee: if no anchor was found, drop the floating btn.
    setTimeout(() => {
      if (!document.getElementById("__cfc_backend_btn")) installFloatingBtn()
    }, 3000)
  }
}
