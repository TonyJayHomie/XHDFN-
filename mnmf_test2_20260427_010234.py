...
class MultiC2Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _forward_v1(self, method: str, body: bytes):
        ...
        Per-backend modelAlias applied before forwarding.
        ...
        for backend in _pick_backends(model):
            ...
            # apply per-backend model alias, then fall back to global merged alias
            send_body = body
            if send_body and method in ("POST", "PUT", "PATCH"):
                try:
                    parsed = json.loads(send_body)
                    # per-backend alias wins; global merged alias is the fallback
    ...
    def do_POST(self):
        ...
        if p.startswith("/api/backends"):
            try:
                ...
                # normalise -- ensure required fields exist on every backend
                for b in bs:
