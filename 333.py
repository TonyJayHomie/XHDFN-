...
ONLY change vs prior 113.py:
  The chrome.runtime.sendMessage interceptor in the injected request.js is
  rewritten from `async function` returning a Promise (which fired callbacks
  synchronously inside the resolved Promise during React render and caused
  React #185) to a sync function that:
    - tries the real SW first via __origSendMessage(...)
    - only falls back on a synchronous throw
    - uses setTimeout(cb, 0) so callbacks always fire on the next event-loop
      tick, never inside React's render phase
"""
...
def write_sanitized_request_js():
    ...
    #  • sendMessage interceptor → SYNC now, real-SW-first, setTimeout fallback
    #                              (fixes React #185 caused by Promise-resolve-
    #                              during-render in the prior async version)
...
// ── sendMessage error recovery (window/sidepanel context only) ───────────────
// MV3 SW sleeps between events. sendMessage fails with "Could not establish
// connection" when idle. Sidepanel error handler then calls setState(isValid
// = false) → re-render → sendMessage → error → loop → React #185.
//
// FIX (replaces prior async-Promise-returning interceptor that caused #185
// by resolving Promises during React render):
//   • Sync function (matches the real chrome.runtime.sendMessage contract,
//     which is callback-style and returns undefined, not a Promise)
//   • ALWAYS try the real SW first via __origSendMessage(...)
//   • Only fall back on a synchronous throw (rare)
//   • Use setTimeout(cb, 0) so callbacks fire on the next event-loop tick,
//     never inside React's render phase
if (globalThis.window && chrome?.runtime?.sendMessage) {
  ...
  chrome.runtime.sendMessage = function(...args) {
    ...
    // Always try the real SW first — the SW's native handlers do the right thing
    try {
      return __origSendMessage(...args)
    } catch(e) {
      // Synchronous throw — fall through to local fallback
    }

    // SW threw synchronously. Answer locally with forced async delay so the
    // callback fires outside React's render cycle (this is what prevents #185).
    if (!isExternal && msg?.type === "check_and_refresh_oauth") {
      ...
      return  // mimic real sendMessage void return
    }

    // Anything else that threw synchronously — swallow with deferred undefined cb
    setTimeout(() => {
      try { if (typeof cb === "function") cb(undefined) } catch(e) {}
    }, 0)
  }
}
