...
def _build_worker_script(local_port: int = CFC_PORT) -> str:
    ...
    return r"""// cocodem-replacement.worker.js  (generated by CFC9.py)
// Drop-in Cloudflare Worker replacing openclaude.111724.xyz + cfc.aroic.workers.dev.
// Deploy: paste into a new Cloudflare Worker, save & deploy.
// Set REMOTE_BASE in CFC9.py to the resulting *.workers.dev URL.

const EXTENSION_ID = "fcoeoabgfenejglbffodgkkbkcdhcgfn";
const LOCAL_CFC    = """ + f'"{local_cfc}";' + r"""

// Static identity — stable values prevent chrome.storage.onChanged from firing
// on every token refresh, which is the root cause of React error #185.
...
const text      = (s, status=200, ct="text/plain") => new Response(s, { status, headers: { "Content-Type": ct, ...CORS } });
...
const ORGANIZATION = {
  ...
  capabilities: ["chat","claude_pro_plan","claude_max_plan","api","computer_use","claude_for_chrome","raven","mcp","browser_extension","tier_max_5x"],
  rate_limit_tier: "default_claude_max", settings: {},
  created_at: "2024-01-01T00:00:00Z",
  active_subscription: { plan: "claude_max", status: "active", type: "claude_max",
    billing_period: "monthly", current_period_start: "2024-01-01T00:00:00Z",
    current_period_end: "2099-12-31T23:59:59Z" },
};
const PROFILE = {
  ...ACCOUNT, account: ACCOUNT, organization: ORGANIZATION, organizations: [ORGANIZATION],
  memberships: [{ organization: ORGANIZATION, role: "admin", joined_at: "2024-01-01T00:00:00Z" }],
...
const ACCOUNT_SETTINGS = {
  ...
  system_prompt: "",
  selected_models: ["claude-opus-4-6","claude-sonnet-4-5-20250929","claude-haiku-4-5-20251001"],
  settings: { theme: "system", language: "en-US" },
};
const TOKEN_PAYLOAD = {
  access_token: TOKEN_STATIC, refresh_token: TOKEN_STATIC,
...
// On SW startup: read previously-saved apiBaseUrl from chrome.storage.local
// so LM Studio / local backend routing works immediately without waiting for
// the user to re-open the backend settings UI.
if (typeof chrome !== "undefined" && chrome.storage?.local?.get) {
  chrome.storage.local.get(["hijackSettings", "ANTHROPIC_BASE_URL"])
    .then(stored => {
      const url = stored.hijackSettings?.backendUrl || stored.ANTHROPIC_BASE_URL || ""
      if (url && globalThis.__cfc_options && !globalThis.__cfc_options.apiBaseUrl) {
        globalThis.__cfc_options.apiBaseUrl = url
      }
    }).catch(() => {})
}
...
if (chrome?.storage?.onChanged?.addListener) {
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === "local") {
      const backendUrl = changes.hijackSettings?.newValue?.backendUrl
                      || changes.ANTHROPIC_BASE_URL?.newValue
      if (backendUrl) {
        // Update SW in-memory options immediately so the next fetch() call
        // routes to the correct apiBase without waiting for a getOptions() refresh.
        if (globalThis.__cfc_options) globalThis.__cfc_options.apiBaseUrl = backendUrl
        try { if (globalThis.localStorage) globalThis.localStorage.setItem("apiBaseUrl", backendUrl) } catch(e) {}
      }
    }
  })
}
  ...
  if (location.pathname == "/options.html") {
    const _observer = new MutationObserver(() => {
      ...
      allItems.forEach(el => {
        ...
        if (t.includes("log out") || t === "logout" || t.includes("sign out") || t === "signout") logoutEl = el
      })
      if (!logoutEl) return
...
class MultiC2Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _forward_v1(self, method: str, body: bytes):
        ...
        for backend in _pick_backends(model):
            ...
            # Only forward the incoming auth token if the backend has its own
            # key configured, OR if the backend is a remote (non-local) URL.
            # Never forward the extension's OAuth token (cfc-local-permanent…)
            # to LM Studio / other local backends — they reject it with 401.
            _url = backend.get("url", "")
            _is_local = any(h in _url for h in ["localhost", "127.0.0.1", "0.0.0.0"])
            if backend.get("key"):
                hdrs["Authorization"] = f"Bearer {backend['key']}"
            elif ext_auth and not _is_local:
