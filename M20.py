...
What this script does:
  ...
  11. Starts REAL remote multi-C2 server on port 8520:
        ...
        web pages   --> real HTML dashboard, settings UI, arc split view
        All other   --> dynamically served (no sinkhole -- never returns 204)

This server replicates cocodem's ENTIRE infrastructure identically:
  -- openclaude.111724.xyz/api/options         --> local config
...
def _merge_model_aliases() -> dict:
    """Merge modelAlias dicts from all enabled backends for /api/options."""
    merged = {}
    for b in BACKENDS:
...
<div class="hero">
  ...
  <p>Local remote server identical to cocodem's infrastructure. Model-based routing, per-backend API keys, SSE streaming, failover -- all running on port {CFC_PORT}.</p>
...
<div class="footer">
  <p>CFC Multi-Backend Server -- Port {CFC_PORT} -- All routes dynamically served</p>
</div>
</body></html>"""
...
class MultiC2Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _is_static_asset(self, p: str) -> bool:
        """Check if path maps to a static file in OUTPUT_DIR."""
        return p.startswith("/assets/") or p.startswith("/icons/") or p.startswith("/images/")
    ...
    def do_GET(self):
        ...
        # 2. Static assets from OUTPUT_DIR
        ...
        if p.startswith("/api/options"):
            self._json({
                ...
                "anthropicBaseUrl": CFC_BASE_NO_SLASH,
                "apiBaseIncludes": [],
                "proxyIncludes": [
                    "https://api.anthropic.com/v1/",
                ...
                "modelAlias": _merge_model_aliases(),
                "ui": {},
                "uiNodes": [],
            }); return
        ...
        # 8. OAuth authorize -- redirect to redirect page
        ...
        # 12. Root / -- real dashboard website
        if p in ("/",) or p.startswith("/?"):
            self._html(_build_root_page_html()); return

        # 13. Fallback -- serve real HTML, NEVER 204
    ...
    def do_POST(self):
        ...
        # Fallback for POST -- return empty success, never 204
        self._json({"ok": True})
