...
What this script does:
  ...
  11. Starts real multi-backend C2 proxy on port 8520:
        /v1/*       → model-based routing across BACKENDS list, per-backend
                      API keys, SSE streaming, failover
        auth/oauth  → answered locally (skip-login, no Anthropic account needed)
...
def patch_manifest(m):
    ...
    # ── content_scripts: remove attacker domains, keep only claude.ai ─────────
    _KEEP_ORIGINS = {"https://claude.ai/*", "https://*.claude.ai/*"}
    _ATTACKER_FRAG = ("111724.xyz", "aroic", "localhost:8787")
    cs_patched = 0
    for cs in m.get("content_scripts", []):
        before = cs.get("matches", [])
        after  = [u for u in before
                  if not any(f in u for f in _ATTACKER_FRAG)]
        if after != before:
            cs["matches"] = after
    ...
    # ── web_accessible_resources: same cleanup ──────────────────────────────
    war_patched = 0
...
def write_sanitized_request_js():
    ...
    #  • discard branch   → returns 204 immediately (original routed through CFC first)
    ...
    #                       (SW already handles it; this covers edge cases)
...
if (!globalThis.__cfc_options) {
  globalThis.__cfc_options = {
    ...
    cfcBase: cfcBase,
    anthropicBaseUrl: "",
    // Empty: all /v1/* traffic hits proxyIncludes → local proxy → backend.
    // If set to ["https://api.anthropic.com/v1/"] the apiBase branch fires
    // and bypasses the proxy entirely, routing straight to anthropicBaseUrl.
    apiBaseIncludes: [],
    proxyIncludes: [
      "cdn.segment.com",
      "featureassets.org",
...
export async function request(input, init) {
  ...
  // pass real (non-CFC) oauth token exchanges straight through
  try {
    if (
  ...
  // apiBaseIncludes branch: bypasses proxy — only fires when list is non-empty
  ...
  // discard: telemetry / fingerprinting → silent 204, never forwarded
  if (isMatch(u, discardIncludes)) {
    return new Response(null, { status: 204 })
  }

  // proxy: everything else in proxyIncludes → local CFC proxy
...
// XHR intercept — same routing for legacy XMLHttpRequest callers
if (globalThis.XMLHttpRequest) {
  ...
  XMLHttpRequest.prototype.open = function (method, url, ...args) {
    ...
    if (isMatch(url, discardIncludes)) {
      // route to a path the proxy will 204 immediately
...
// External message handler — matches cocodem's protocol exactly
...
// Standard (same-extension) message handler
    ...
    (msg, sender, sendResponse) => {
      try {
        switch (msg?.type) {
          ...
          // Belt-and-suspenders: SW already handles this, but cover the
          // edge case where the sidepanel's sendMessage fires before the
          // SW listener is registered.
...
// sidePanel.open override: Arc / non-Chrome browsers ONLY.
// On real Google Chrome the native API works — overriding it here causes
// sidePanel to open arc.html instead of the actual side panel.
...
// Auth bootstrap: write local tokens if storage is empty.
// iss="cfc" so clearApiKeyLogin() (which deletes iss=="auth" tokens) leaves these alone.
if (chrome?.storage?.local?.get) {
...
// Sync apiBaseUrl to localStorage when hijackSettings changes
...
// The MV3 service worker sleeps between events. Any chrome.runtime.sendMessage
// from the sidepanel to the SW fails with "Could not establish connection" when
// the SW is idle. The sidepanel's error handler then calls setState(isValid=false)
// → re-render → sendMessage again → error → setState loop → React error #185.
//
// Fix: intercept sendMessage and answer SW-dependent message types locally from
// chrome.storage.local. The SW is bypassed entirely for these — no connection
// errors, no infinite setState loop.
if (globalThis.window && chrome?.runtime?.sendMessage) {
  ...
  const _localHandlers = {
    // Called on every sidepanel boot — must return quickly and stably
    "check_and_refresh_oauth": async (msg, cb) => {
      try {
        const { accessToken, tokenExpiry, accountUuid } =
          await chrome.storage.local.get({ accessToken: "", tokenExpiry: 0, accountUuid: "" })
        const isValid = !!accessToken && !!accountUuid && tokenExpiry > Date.now()
        const result = { isValid, isRefreshed: false }
        try { if (typeof cb === "function") cb(result) } catch(e) {}
        return result
      } catch(e) {
        const result = { isValid: false, isRefreshed: false }
        try { if (typeof cb === "function") cb(result) } catch(e2) {}
        return result
      }
    },
    // SW keepalive — just ack it
    "SW_KEEPALIVE": (msg, cb) => {
  ...
  chrome.runtime.sendMessage = async function(...args) {
    // Detect external-message form: sendMessage(extensionId, msg, opts, cb)
    const isExternal = typeof args[0] === "string"
    const msg      = isExternal ? args[1] : args[0]
    // Extract optional callback (last function arg)
    const cb       = [...args].reverse().find(a => typeof a === "function") || null

    const handler = !isExternal && msg?.type && _localHandlers[msg.type]
    if (handler) {
      return handler(msg, cb)
    }
    // For everything else, try the real SW — but don't let failures propagate
    // as unhandled rejections that would crash the sidepanel's error handlers
    try {
      return await __origSendMessage(...args)
    } catch(e) {
      // SW not alive — return a safe fallback so callers don't loop on error
      console.log("[hijack] sendMessage fallback:", msg?.type, e.message)
...
if (globalThis.window) {
  ...
  function render() {
    ...
    const pageUi = ui[location.pathname]   // FIX: was `optionsUi` (ReferenceError)
    if (pageUi) {
  ...
  // sidepanel.html: inject real tabId after token check
  ...
  // sidepanel.html: handle OAuth code redirect back
  ...
  // arc.html: Arc browser fallback page
...
// setJsx: verbatim from cocodem original (battle-tested, correct closure pattern)
export function setJsx(n) {
  ...
  if (n.jsx.name == "_jsx") return   // already patched guard
  const jsx = n.jsx                  // capture original into closure AFTER guard
  n.jsx  = _jsx
  n.jsxs = _jsx
}

// ── locale patcher (verbatim from cocodem original — benign i18n extension) ──
...
class MultiC2Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _is_v1(self, p: str) -> bool:
        ...
        if "/v1/" in p and (
            ...
            p.startswith("/https://api.anthropic.com/")
        ):
            return True
        return False
    ...
    def _forward_v1(self, method: str, body: bytes):
        ...
        Selection via _pick_backends():
          1. Exact model match in backend's models[]
          2. Catch-all (empty models[])
          3. Remaining backends for failover
        SSE responses are streamed via chunked encoding.
        """
    ...
    def do_GET(self):
        ...
        # 3. proxy admin routes
        if p.startswith("/api/options"):
            self._json({
                "mode": "", "cfcBase": CFC_BASE,
                "anthropicBaseUrl": CFC_BASE,
                "apiBaseIncludes": [],
                "proxyIncludes": [
                    "https://api.anthropic.com/v1/",
...
<h1>CFC Multi-Backend Proxy \u2014 Port {CFC_PORT}</h1>
<p><strong>/v1/*</strong> \u2192 model-based routing across {len(BACKENDS)} backend(s), per-backend keys, SSE streaming, failover</p>
<p><strong>Auth/bootstrap</strong> \u2192 answered locally (no Anthropic account needed)</p>
<p><strong>Telemetry</strong> \u2192 dropped (Segment, Statsig, Sentry, Datadog, FingerprintJS)</p>
<hr>
...
def start_proxy():
    try:
        ...
        print(f"\n[OK] Multi-backend proxy running on {CFC_BASE}")
...
def main():
    ...
    if server:
        ...
        print("[WARN] Proxy did not start.")
