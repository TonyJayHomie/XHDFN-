...
Dual-domain replacement architecture:
  REMOTE: test2.mahnikka.workers.dev  =  openclaude.111724.xyz  AND  cfc.aroic.workers.dev
          One CF Worker replaces both attacker domains. Serves /api/options, all auth
          stubs, /licenses/verify, /oauth/authorize. 307s everything else to LOCAL.
  LOCAL:  localhost:8520               =  localhost:8787
          Python proxy handles /v1/* inference forwarding, static files, backend
          settings UI, auth stubs, telemetry 204s.

cfcBase in request.js: "https://test2.mahnikka.workers.dev/" || "http://localhost:8520/" || ""
Matches original cocodem dual-domain pattern exactly with clean infrastructure.
"""
...
def _load_backends():
    if BACKENDS_FILE.exists():
        try:
            ...
            pass
    return [{"name":"Default","url":DEFAULT_BACKEND_URL,"key":"","models":[],"modelAlias":{},"enabled":True}]
...
_DEFAULT_IDENTITY = {
    "apiBaseUrl":"http://127.0.0.1:1234/v1","apiKey":"","authToken":"",
    ...
    if IDENTITY_FILE.exists():
        try:
            ...
            if isinstance(d, dict):
                m = dict(_DEFAULT_IDENTITY); m.update(d)
                if not isinstance(m.get("modelAliases"), dict): m["modelAliases"] = {}
                return m
        except Exception:
            pass
...
# ─── Cloudflare Worker — full server (replaces 111724.xyz AND cfc.aroic) ──────

def _build_worker_script(local_port):
...
const ORG = {
  ...
  rate_limit_tier:"local",settings:{},created_at:"2024-01-01T00:00:00Z",
};
const PROFILE = {
  ...ACCOUNT,account:ACCOUNT,organization:ORG,organizations:[ORG],
...
const BOOTSTRAP = {
  ...
  active_subscription:{
    plan:"local_cfc",status:"active",type:"local_cfc",billing_period:"none",
    current_period_start:"2024-01-01T00:00:00Z",
    current_period_end:"2099-12-31T23:59:59Z",
  },
  chat_enabled:true,
  capabilities:["chat","api","computer_use","claude_for_chrome"],
  rate_limit_tier:"local",settings:{theme:"system",language:"en-US"},
};
...
function routeAuth(raw){
  if(raw.includes("oauth/token"))        return jresp(tokenResponse());
  if(raw.includes("oauth/profile"))      return jresp(PROFILE);
  if(raw.includes("oauth/account"))      return jresp(PROFILE);
  if(raw.includes("bootstrap"))          return jresp(BOOTSTRAP);
  if(raw.includes("organizations"))      return jresp([ORG]);
  if(raw.includes("chat_conversations")) return jresp({conversations:[],limit:0,has_more:false,cursor:null});
  if(raw.includes("licenses/verify"))    return jresp({valid:true,license:"local",tier:"local",expires:"2099-12-31"});
  if(raw.includes("domain_info"))        return jresp({domain:"local",allowed:true});
  if(raw.includes("url_hash_check"))     return jresp({allowed:true});
  if(raw.includes("usage"))             return jresp({usage:{},limit:null});
  if(raw.includes("entitlements"))       return jresp({entitlements:[]});
  if(raw.includes("flags"))             return jresp({});
  if(raw.includes("spotlight"))          return jresp({items:[],total:0});
  if(raw.includes("mcp/v2"))            return jresp({servers:[],tools:[],enabled:false});
  return null;
}
...
a.btn{display:inline-flex;align-items:center;justify-content:center;height:48px;padding:0 32px;background:#c45f3d;color:white;text-decoration:none;border-radius:12px;font-size:16px;font-weight:800}
...
<body><div class="box">
  ...
  <a class="btn" href="${esc(next.toString())}">Continue</a>
</div></body></html>`);
}
...
export default {
  async fetch(request){
    ...
    if(path==="/"||path==="/backend_settings"){
      return hresp(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>CFC Worker</title></head>
<body style="font-family:sans-serif;background:#f9f8f3;padding:32px">
<h1>CFC Worker — Online</h1><p>${new Date().toISOString()}</p>
<p>Local: <code>${LOCAL_CFC}</code></p></body></html>`);
    }
    ...
    if(path==="/api/options")
      return jresp(optionsPayload(url.origin+"/"));
    if(path==="/api/identity")
      return jresp({apiBaseUrl:LOCAL_BACKEND,apiKey:"",authToken:"",
        email:"user@local",username:"local-user",licenseKey:"",
        blockAnalytics:true,modelAliases:{},mode:""});
    if(path==="/api/backends")
      return jresp({backends:[{name:"Default",url:LOCAL_BACKEND,key:"",models:[],modelAlias:{},enabled:true}]});
...
def _deploy_cloudflare_worker(api_token, account_id, worker_name, script):
    url = f"https://api.cloudflare.com/client/v4/accounts/{account_id}/workers/scripts/{worker_name}"
    req = urllib.request.Request(url, data=script.encode("utf-8"),
        headers={"Authorization":f"Bearer {api_token}","Content-Type":"application/javascript"},
        method="PUT")
    try:
        ...
        return {"success":False,"errors":[{"message":f"HTTP {e.code}: {body}"}]}
    except Exception as ex:
        return {"success":False,"errors":[{"message":str(ex)}]}

def _get_workers_subdomain(api_token, account_id):
...
def copy_source():
    if not COCODEM_SRC.exists():
        print(f"[ERROR] Source not found: {COCODEM_SRC}"); sys.exit(1)
...
def read_manifest():
    with open(OUTPUT_DIR/"manifest.json","r",encoding="utf-8") as f: m=json.load(f)
    print(f"\n[OK] manifest.json: {m.get('name')} v{m.get('version')}")
    return m

def patch_manifest(m):
    changes=[]
    if "update_url" in m: del m["update_url"]; changes.append("REMOVED update_url")
    hp=m.get("host_permissions",[])
    for h in ["http://127.0.0.1/*","http://localhost/*","http://*/*"]:
        if h not in hp: hp.append(h)
    m["host_permissions"]=hp; changes.append("ADDED localhost host_permissions")
    perms=m.get("permissions",[])
    if "storage" not in perms: perms.append("storage"); m["permissions"]=perms
    csp=m.get("content_security_policy",{})
    if isinstance(csp,dict):
        policy=csp.get("extension_pages","")
        if "connect-src" in policy:
            policy=policy.replace("connect-src",
                "connect-src http://localhost:* http://127.0.0.1:* http://*:* wss://bridge.claudeusercontent.com")
        else:
    ...
    for cs in m.get("content_scripts",[]):
        before=cs.get("matches",[]); after=[u for u in before if not any(f in u for f in _ATTACKER)]
        if after!=before: cs["matches"]=after; cs_p+=len(before)-len(after)
    if cs_p: changes.append(f"REMOVED {cs_p} attacker URL(s) from content_scripts.matches")
    wp=0; wla=0
    for war in m.get("web_accessible_resources",[]):
        before=war.get("matches",[]); after=[u for u in before if not any(f in u for f in _ATTACKER)]
        if after!=before: wp+=len(before)-len(after)
        for lm in _LOCAL:
            if lm not in after: after.append(lm); wla+=1
        war["matches"]=after
    if wp:  changes.append(f"REMOVED {wp} attacker URL(s) from web_accessible_resources.matches")
    if wla: changes.append(f"ADDED {wla} localhost match(es) to web_accessible_resources.matches")
...
def write_sanitized_request_js():
    ...
    # CFCBASE = REMOTE_BASE (test2 Worker) || LOCAL (localhost:8520) || ""
    # Mirrors original: openclaude.111724.xyz || localhost:8787 || ""
    # test2 Worker replaces BOTH 111724.xyz AND cfc.aroic.workers.dev
    clean = r"""
...
const cfcBase = "CFCBASE_REMOTE" || "CFCBASE_LOCAL" || ""
...
"""
    # Wire in actual URLs
    clean = clean.replace("CFCBASE_REMOTE", REMOTE_BASE)
    clean = clean.replace("CFCBASE_LOCAL",  CFC_BASE)

    cocodem_req.write_text(clean, encoding="utf-8")
    print(f"[OK] assets/request.js")
    print(f"     cfcBase: \"{REMOTE_BASE}\" || \"{CFC_BASE}\" || \"\"")
...
def write_backend_settings_ui():
  ...
  const F = (id, label, type, ph, icon, note) =>
    `<div style="margin-bottom:16px">
      <label style="display:block;font-size:9px;font-weight:900;color:#8B856C;text-transform:uppercase;letter-spacing:.15em;margin:0 0 6px 4px">
        ${label}${note?`<span style="color:#2d6a4f;font-weight:600;text-transform:none;letter-spacing:0;margin-left:4px">${note}</span>`:""}
      </label>
      <div style="position:relative">
        <div style="position:absolute;inset:0 auto 0 14px;display:flex;align-items:center;color:#b4af9a;pointer-events:none">${icon}</div>
        <input id="${id}" type="${type}" placeholder="${ph}"
          style="width:100%;height:44px;padding:0 14px 0 38px;border:1px solid #e5e2d9;background:#fcfbf9;color:#1d1b16;border-radius:12px;font-size:14px;font-weight:500;box-sizing:border-box;outline:none;transition:border-color .2s,box-shadow .2s">
      </div>
    </div>`;
  container.innerHTML=`<div style="min-height:100vh;width:100%;background:#f9f8f3;color:#3d3929;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;display:flex;align-items:flex-start;justify-content:center;padding:32px 20px">
    <div style="background:white;border:1px solid #e5e2d9;width:100%;max-width:520px;padding:36px;border-radius:40px;box-shadow:0 12px 40px rgba(0,0,0,.02)">
      <div style="text-align:center;margin-bottom:24px">
        <div style="display:inline-flex;align-items:center;justify-content:center;width:72px;height:72px;border-radius:20px;margin-bottom:16px;background:#fcfbf9;border:1px solid #e5e2d9">
          <img src="${ICON}" alt="Claude" style="width:36px;height:36px;border-radius:6px">
        </div>
        <h1 style="font-size:26px;font-family:'Iowan Old Style',Georgia,serif;color:#1d1b16;letter-spacing:-.02em;margin:0 0 8px;font-weight:400">Backend Settings</h1>
        <p style="color:#6b6651;font-size:13px;font-weight:500;margin:0;line-height:1.5">All data stored locally. <span style="color:#2d6a4f;font-weight:700">\u2714 No calls to cocodem servers.</span></p>
      </div>
      <div id="bs_st" style="padding:11px 16px;border-radius:10px;margin-bottom:18px;display:none;font-size:13px;font-weight:600"></div>
      <div style="font-size:9px;font-weight:900;color:#8B856C;text-transform:uppercase;letter-spacing:.15em;margin-bottom:14px">\u25b8 API Configuration</div>
      ...
      <div style="font-size:9px;font-weight:900;color:#8B856C;text-transform:uppercase;letter-spacing:.15em;margin:22px 0 8px">\u25b8 Model Aliases <span style="color:#b4af9a;font-weight:600;text-transform:none;letter-spacing:0;margin-left:6px">(JSON, optional)</span></div>
      <textarea id="bs_aliases" placeholder='{"claude-opus-4-7":"local-model-name"}' style="width:100%;min-height:76px;padding:11px 14px;margin-bottom:4px;border:1px solid #e5e2d9;background:#fcfbf9;color:#1d1b16;border-radius:12px;font-size:13px;font-family:monospace;box-sizing:border-box;outline:none;resize:vertical"></textarea>
      <div style="font-size:9px;font-weight:900;color:#8B856C;text-transform:uppercase;letter-spacing:.15em;margin:22px 0 14px">\u25b8 Identity <span style="color:#2d6a4f;font-weight:700;text-transform:none;letter-spacing:0;margin-left:6px">\u2714 local \u2014 no license server</span></div>
      ...
      <div style="display:flex;gap:10px;margin:22px 0 14px">
        <button id="bs_save" style="flex:1;height:44px;background:#c45f3d;color:white;border:none;border-radius:12px;font-size:15px;font-weight:900;cursor:pointer">Save Settings</button>
        <button id="bs_test" style="flex:1;height:44px;background:white;color:#3d3929;border:1px solid #e5e2d9;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer">Test Connection</button>
      </div>
      <div style="text-align:center">
        <button id="bs_adv" style="font-size:9px;font-weight:900;color:#b4af9a;text-transform:uppercase;letter-spacing:.18em;background:none;border:none;cursor:pointer;display:inline-flex;align-items:center;gap:7px;padding:8px 12px">${K.cog} Advanced Options</button>
        <div id="bs_ap" style="display:none;margin-top:12px;padding:18px;background:#fcfbf9;border-radius:14px;border:1px solid #e5e2d9;text-align:left">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px">
            <input type="checkbox" id="bs_block" checked style="width:auto;margin:0;accent-color:#c45f3d">
            <label for="bs_block" style="font-size:12px;font-weight:600;color:#3d3929;margin:0;cursor:pointer">Block analytics / telemetry</label>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button id="bs_clear" style="font-size:9px;font-weight:900;color:#b04a3d;text-transform:uppercase;letter-spacing:.15em;background:none;border:1px solid #e5e2d9;border-radius:8px;padding:8px 14px;cursor:pointer;display:inline-flex;align-items:center;gap:7px">${K.trash} Clear Saved Data</button>
            <button id="bs_dump" style="font-size:9px;font-weight:900;color:#3d3929;text-transform:uppercase;letter-spacing:.15em;background:none;border:1px solid #e5e2d9;border-radius:8px;padding:8px 14px;cursor:pointer">Dump Storage (Console)</button>
          </div>
          <pre id="bs_pre" style="display:none;margin-top:12px;padding:12px;background:#1d1b16;color:#e8e3d6;border-radius:8px;font-size:11px;font-family:monospace;max-height:180px;overflow:auto"></pre>
        </div>
      </div>
    </div>
  </div>`;
  ...
  const KEYS=["ANTHROPIC_BASE_URL","ANTHROPIC_API_KEY","ANTHROPIC_AUTH_TOKEN","email","username","licenseKey","hijackSettings"];
  const CFC_PROXY_BASE="http://localhost:8520/";
  let proxyIdentity=null;
  try{const r=await fetch(CFC_PROXY_BASE+"api/identity",{cache:"no-store"});if(r.ok)proxyIdentity=await r.json();}catch(e){}
  ...
  $("bs_aliases").value=aliasSrc&&Object.keys(aliasSrc).length?JSON.stringify(aliasSrc,null,2):"";
  if(proxyIdentity)st("\u2714 Loaded from proxy at "+CFC_PROXY_BASE);
  $("bs_save").onclick=async()=>{
    ...
    const payload={apiBaseUrl:base,apiKey:$("bs_key").value.trim(),authToken:$("bs_auth").value.trim(),
      email:$("bs_email").value.trim()||"user@local",username:$("bs_user").value.trim()||"local-user",
      licenseKey:$("bs_lic").value.trim(),blockAnalytics:$("bs_block").checked,modelAliases:ma};
    let proxyOk=false;
    try{
      const r=await fetch(CFC_PROXY_BASE+"api/identity",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
      const d=await r.json();proxyOk=!!d.ok;
    }catch(e){st("\u2717 Proxy unreachable -- saved to chrome.storage only: "+e.message,"w");}
    await chrome.storage.local.set({
      ANTHROPIC_BASE_URL:base,ANTHROPIC_API_KEY:$("bs_key").value.trim(),
    ...
    try{if(chrome?.runtime?.sendMessage)chrome.runtime.sendMessage({type:"_update_options"},()=>{});}catch(e){}
    if(proxyOk)st("\u2714 Saved. Backend: "+base);
    else st("\u26a0 Saved locally only. Backend: "+base,"w");
  };
  ...
  $("bs_clear").onclick=async()=>{
    if(!confirm("Clear all saved Backend Settings?"))return;
    try{await fetch(CFC_PROXY_BASE+"api/identity",{method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({apiBaseUrl:"http://127.0.0.1:1234/v1",apiKey:"",authToken:"",
        email:"user@local",username:"local-user",licenseKey:"",blockAnalytics:true,modelAliases:{}})});}catch(e){}
    await chrome.storage.local.remove(KEYS);
    try{localStorage.removeItem("apiBaseUrl");}catch(e){}
    st("\u2714 Cleared");setTimeout(()=>location.reload(),600);
  };
...
<body>
  <div id="root">
    ...
    (OUTPUT_DIR/"arc.html").write_text(h, encoding="utf-8")
    print("[OK] arc.html")
...
def write_options():
    for html_file in [OUTPUT_DIR/"sidepanel.html", OUTPUT_DIR/"options.html"]:
        ...
        for line in lines:
            ...
            elif "</script>" in line and skip: skip=False
...
<body><p>Redirecting to <a href="/options.html#backendsettings">Backend Settings</a>...</p></body>
    ...
    if bs.exists() and not og.exists(): shutil.copy2(bs,og); print("[OK] backend_settings.html --> backend_settingsOG.html")
    bs.write_text(stub, encoding="utf-8")
    if not og.exists(): og.write_text(stub, encoding="utf-8")
    print("[OK] backend_settings.html (meta-refresh stub)")


def inject_index_module():
    assets=OUTPUT_DIR/"assets"; target=None
    known=assets/"index-BVS4T5_D.js"
    if known.exists(): target=known; print(f"  jsx-runtime: {target.relative_to(OUTPUT_DIR)} (known)")
    if not target:
        sp=OUTPUT_DIR/"sidepanel.html"
        if sp.exists():
            h=sp.read_text(encoding="utf-8")
            for href in re.findall(r'<link[^>]+rel="modulepreload"[^>]+href="([^"]+)"',h):
                ...
                if c.exists() and c.stat().st_size<30000:
                    ...
                    if "jsx" in t and "jsxs" in t and "Fragment" in t and "$$typeof" in t:
                        target=c; print(f"  jsx-runtime: {target.relative_to(OUTPUT_DIR)} (modulepreload)"); break
    if not target:
        for f in sorted(assets.glob("index-*.js")):
            if f.stat().st_size<30000:
                t=f.read_text(encoding="utf-8")
                if "jsx" in t and "jsxs" in t and "Fragment" in t:
                    target=f; print(f"  jsx-runtime: {target.relative_to(OUTPUT_DIR)} (scan)"); break
    if not target: print("[WARN] jsx-runtime not found -- setJsx not injected"); return
    content=target.read_text(encoding="utf-8")
    if "setJsx" in content: print(f"[OK] setJsx already in {target.relative_to(OUTPUT_DIR)} -- skip"); return
    ...
    if b: content=content[:b.end(1)]+injection+content[b.start(2):]
    else:
        fn=re.search(r"function\s+d\s*\(",content)
        if fn: content=content[:fn.start()]+injection+content[fn.start():]
...
def local_token():
    # Static iat:1700000000 -- dynamic iat caused React error #185
    tok = _b64url({"alg":"none","typ":"JWT"}) + "." + \
          _b64url({"iss":"cfc","sub":USER_UUID,"exp":int(time.time())+315360000,"iat":1700000000}) + ".local"
    return {"access_token":tok,"token_type":"bearer","expires_in":315360000,
            "refresh_token":"local-refresh","scope":"user:profile user:inference user:chat"}
...
def profile_payload():
    ...
    acct["full_name"]=acct["name"]=acct["display_name"]=IDENTITY.get("username") or "Local User"
    return {**acct,"account":acct,"organization":LOCAL_ORG,"organizations":[LOCAL_ORG],
            "memberships":[{"organization":LOCAL_ORG,"role":"admin","joined_at":"2024-01-01T00:00:00Z"}],
            "active_organization_uuid":ORG_UUID}

def bootstrap_payload():
    prof=profile_payload()
    return {**prof,"account_uuid":USER_UUID,
            "statsig":{"user":{"userID":USER_UUID,"custom":{"organization_uuid":ORG_UUID}},
                       "values":{"feature_gates":{},"dynamic_configs":{},"layer_configs":{}}},
            "flags":{},"features":[],"active_flags":{},
            "active_subscription":{"plan":"local_cfc","status":"active","type":"local_cfc",
                                   "billing_period":"none","current_period_start":"2024-01-01T00:00:00Z",
                                   "current_period_end":"2099-12-31T23:59:59Z"},
            "chat_enabled":True,
            "capabilities":["chat","api","computer_use","claude_for_chrome"],
            "rate_limit_tier":"local","settings":{"theme":"system","language":"en-US"}}
...
<style>body{{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f9f8f3;color:#2f2a1f;margin:0;padding:32px}}
...
a.btn{{display:inline-flex;align-items:center;height:40px;padding:0 16px;background:#c45f3d;color:white;text-decoration:none;border-radius:10px;font-weight:700}}</style></head>
<body><div class="box">
  <h1>CFCBASE Local Server</h1>
  <p>Local: <code>{CFC_BASE}</code> &middot; Remote: <code>{REMOTE_BASE}</code></p>
  <p><a class="btn" href="{BACKEND_SETTINGS_URL}">Backend Settings</a></p>
  <ul>
    <li><code>/api/options</code> &mdash; live from IDENTITY + BACKENDS</li>
    <li><code>/oauth/redirect</code> &mdash; writes tokens via _set_storage_local</li>
    <li><code>/v1/*</code> &mdash; forwarded to backend(s) with failover</li>
    <li>All auth routes answered locally &middot; Telemetry &rarr; 204</li>
  </ul>
</div></body></html>"""
...
def authorize_html(query):
    ...
    if qs_pairs: next_url+="?"+urlencode(qs_pairs)
    next_attr=next_url.replace("&","&amp;").replace('"',"&quot;")
    return f"""<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Claude in Chrome</title>
<style>*{{box-sizing:border-box}}body{{background:#f9f8f3;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;color:#1d1b16}}
.box{{background:white;border:1px solid #e5e2d9;border-radius:32px;padding:40px;max-width:420px;width:100%;text-align:center}}
.logo{{width:72px;height:72px;border-radius:20px;border:1px solid #e5e2d9;margin:0 auto 20px;display:flex;align-items:center;justify-content:center;color:#d97757;font-size:38px}}
h1{{font-family:Georgia,serif;font-weight:400;margin:0 0 10px;font-size:28px}}p{{color:#6b6651;font-size:14px;line-height:1.5;margin:0 0 24px}}
a.btn{{display:inline-flex;align-items:center;justify-content:center;height:48px;padding:0 32px;background:#c45f3d;color:white;text-decoration:none;border-radius:12px;font-size:16px;font-weight:800;border:0}}</style></head>
<body><div class="box">
  <div class="logo">&#10038;</div>
  <h1>Claude in Chrome</h1>
  <p>Local CFC. No Anthropic credentials required.</p>
  <a class="btn" href="{next_attr}">Continue</a>
</div></body></html>"""
...
def redirect_html(query):
    ...
    storage={
        ...
        "tokenExpiry":int(time.time()*1000)+31536000000,"accountUuid":USER_UUID,
        "sidepanelToken":code,"sidepanelTokenExpiry":int(time.time()*1000)+31536000000,
        ...
        "hijackSettings":{"backendUrl":IDENTITY.get("apiBaseUrl") or DEFAULT_BACKEND_URL,
                          "modelAliases":_merged_model_alias(),"blockAnalytics":True},
    }
    storage_js=json.dumps(storage); final_uri_js=json.dumps(final_uri); ext_id_js=json.dumps(EXTENSION_ID)
    final_uri_attr=final_uri.replace("&","&amp;").replace('"',"&quot;")
    return f"""<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Signed In</title>
<style>body{{background:#f9f8f3;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;color:#1d1b16}}
...
a.fallback{{display:inline-block;margin-top:16px;color:#c45f3d;font-weight:700}}</style></head>
<body><div class="box">
  <h1>Signed in</h1>
...
(function(){{
  ...
  var msg=document.getElementById("msg"),fb=document.getElementById("fallback");
  function done(text,ok){{
    msg.textContent=text;msg.style.color=ok?"#2d6a4f":"#b04a3d";
...
<style>*{{box-sizing:border-box}}body{{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#19182c;color:#f5f3ee;margin:0;padding:32px 20px;min-height:100vh}}
.wrap{{max-width:980px;margin:0 auto}}h1{{font-size:28px;color:#ff4568;margin:0 0 4px;font-weight:800}}.sub{{color:#7f82a8;margin:0 0 28px}}
.panel{{background:#142447;border:1px solid #20345f;border-radius:12px;padding:22px;margin:18px auto;max-width:760px}}.panel h2{{font-size:16px;color:#ff4568;margin:0 0 16px}}
...
label{{color:#7f82a8;font-size:14px}}input,textarea,select{{width:100%;background:#17172b;color:#f5f3ee;border:1px solid #ef4565;border-radius:6px;padding:11px;font-size:14px;font-family:monospace}}
.actions{{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}}button{{background:#ef4565;color:white;border:0;border-radius:6px;padding:10px 16px;font-weight:700;cursor:pointer}}
button.secondary{{background:#202842;color:#d8d7e8;border:1px solid #303b62}}button.danger{{background:#3a1730;color:#ff4568;border:1px solid #73304c}}
...
.ok{{display:block;background:#164f36;color:#9af0ba}}.err{{display:block;background:#4b1b2b;color:#ff9bb0}}
code{{background:#0f1a32;color:#7ff0b2;border-radius:4px;padding:2px 5px}}</style></head>
<body><div class="wrap">
  <h1>CFC Backend Setup</h1>
  <p class="sub">Remote: <code>{REMOTE_BASE}</code> &nbsp;&middot;&nbsp; Local: <code>{CFC_BASE}</code></p>
  <div id="status" class="status"></div>
  <section class="panel"><h2>API Backends</h2><div id="backendList"></div>
    <button class="secondary" onclick="addBackend()">+ Add Backend</button>
  </section>
  <section class="panel"><h2>Identity</h2>
    <div class="grid"><label>API Base URL</label><input id="apiBaseUrl" value="{IDENTITY.get('apiBaseUrl') or DEFAULT_BACKEND_URL}"></div>
    <div class="grid"><label>Mode</label>
      <select id="mode"><option value="">api (local/custom backend)</option><option value="claude">claude (official)</option></select>
    </div>
    <div class="grid"><label>Auth Token</label><input id="authToken" value="{IDENTITY.get('authToken','')}" placeholder="blank = auto bypass token"></div>
  </section>
  <section class="panel"><h2>Quick Actions</h2>
    <div class="actions">
  ...
  <button class="save" onclick="saveConfig()">Save Config &rarr;</button>
</div>
<script>
...
function renderBackends(){{
  ...
  list.innerHTML=backends.map((b,i)=>`<div class="backend">
    ...
    <div style="color:#7f82a8;font-size:12px;margin-top:6px" id="bs${{i}}"></div>
  </div>`).join("");
}}
...
async function testBackend(i){{
  const b=backends[i],el=document.getElementById("bs"+i);el.textContent="Testing...";
  try{{
...
async function saveConfig(){{
  ...
  showStatus(br.ok&&ir.ok?"Saved":"Save failed \u2014 check server logs",br.ok&&ir.ok);
}}
renderBackends();
</script></body></html>"""
...
class Handler(http.server.BaseHTTPRequestHandler):
    server_version = f"CFCBASE/{TIMESTAMP}"
    ...
    def _json(self, obj, status=200):
        try: self._send(json.dumps(obj).encode("utf-8"),"application/json",status)
        except Exception as e: sys.stderr.write(f"[HANDLER-ERROR] _json: {e}\n"); sys.stderr.flush()

    def _html(self, body, status=200):
        self._send(body.encode("utf-8"),"text/html; charset=utf-8",status)

    def _204(self):
        self.send_response(204); self.send_header("Connection","close"); self._cors(); self.end_headers()
    ...
    def _is_auth(self, p):
        ...
        return any(m in p for m in ["/oauth/","/bootstrap","/domain_info","/chat_conversations",
            "/organizations","/url_hash_check","/api/web/","/features/","/spotlight",
            "/usage","/entitlements","/flags","/mcp/v2/","/licenses/"])
    ...
    def _serve_static(self, path):
        ...
        for c in candidates:
            if c.exists() and c.is_file():
                ...
                self._send(c.read_bytes(),ct); return True
        return False

    def _v1_suffix(self, path):
        if path.startswith("/v1/"): return path[3:]
        idx=path.find("/v1/"); return path[idx+3:] if idx>=0 else path
    ...
    def _forward_v1(self, method, body):
        ...
        allowed={"content-type","accept","authorization","anthropic-version","anthropic-beta",
                 "anthropic-client-platform","anthropic-client-version","x-api-key","x-service-name"}
        ...
        for backend in _pick_backends(model):
            ...
            try:
                req=urllib.request.Request(target,data=send_body or None,headers=hdrs,method=method)
                resp=urllib.request.urlopen(req,timeout=300)
                ct=resp.headers.get("Content-Type","") or "application/json"
                if "text/event-stream" in ct:
                    self.send_response(resp.status); self._stream_sse(resp)
                else:
                    data=resp.read(); self._send(data,ct,resp.status)
                return
            except urllib.error.HTTPError as e:
                data=e.read() or b""
                if e.code<500:
                    self._send(data,e.headers.get("Content-Type") or "application/json",e.code); return
                last_err=f"HTTP {e.code}"
            except Exception as e:
                err=f"{type(e).__name__}: {e}"
                sys.stderr.write(f"[PROXY-ERROR] {err}\n{traceback.format_exc()}\n"); sys.stderr.flush()
                last_err=err
        self._json({"error":{"type":"proxy_error","message":"All backends failed. Last: "+str(last_err)}},502)

    def do_OPTIONS(self):
        self.send_response(200); self.send_header("Connection","close"); self._cors(); self.end_headers()
    ...
    def do_POST(self):
        parsed=urlparse(self.path); path=parsed.path; body=self._body()
        if self._is_tel(self.path): self._204(); return
        if path.startswith("/api/identity"):
            try:
                data=json.loads(body.decode("utf-8") or "{}")
                for k in ["apiBaseUrl","apiKey","authToken","email","username","licenseKey","blockAnalytics","modelAliases","mode"]:
                    if k in data: IDENTITY[k]=data[k]
                if not isinstance(IDENTITY.get("modelAliases"),dict): IDENTITY["modelAliases"]={}
                _save_identity(); self._json({"ok":True,"identity":IDENTITY})
        ...
        if path.startswith("/api/backends"):
            try:
                ...
                if not isinstance(bks,list) or not bks: raise ValueError("backends must be non-empty list")
                for b in bks:
                ...
                BACKENDS[:]=bks; _save_backends(); self._json({"ok":True,"backends":BACKENDS})
            except Exception as e: self._json({"ok":False,"error":str(e)},400)
            return
...
def start_proxy():
    try:
        ...
        print(f"\n[OK] CFCBASE local proxy: {CFC_BASE}")
        ...
        print(f"     /v1/*         : forwarded to backend(s) with failover")
        print(f"     Static assets : {OUTPUT_DIR}")
        return server
    except OSError as e:
        print(f"[WARN] Cannot bind port {CFC_PORT}: {e}"); return None
...
def print_report(m):
    ...
    print(f"\n  1. Disable cocodem in chrome://extensions/")
    print(f"  2. Enable Developer Mode")
    print(f"  3. Load unpacked --> {OUTPUT_DIR.resolve()}")
    print(f"\n  Backend Settings: {BACKEND_SETTINGS_URL}")
    print(f"  Local Proxy:      {CFC_BASE}")
    print(f"  Remote Worker:    {REMOTE_BASE}")
...
def maybe_deploy_worker():
    print("\n"+"="*62)
    print("  Cloudflare Worker Deploy (optional -- press Enter to skip)")
    print("="*62)
    cf_token=input("  CF API Token (Enter to skip): ").strip()
    if not cf_token:
        print(f"  [SKIP] Using existing worker: {REMOTE_BASE}"); return
    cf_account=input("  CF Account ID: ").strip()
    cf_worker=input(f"  Worker name [test2]: ").strip() or "test2"
    if not cf_account:
        print("  [SKIP] Account ID required."); return
    print(f"\n  [..] Building worker script (port: {CFC_PORT})...")
    script=_build_worker_script(CFC_PORT)
    print(f"  [..] Deploying {cf_worker}...")
    result=_deploy_cloudflare_worker(cf_token,cf_account,cf_worker,script)
    if result.get("success"):
        print(f"  [OK] Worker deployed: {REMOTE_BASE}")
    else:
        errs=result.get("errors",[{"message":"unknown"}])
        print("  [ERR] Deploy failed:")
        for e in errs: print(f"    {e.get('message',e)}")
        print("  Continuing with existing worker.")
...
def main():
    print("="*62)
    print(f"  CFC Extension Sanitizer -- {TIMESTAMP}")
    print(f"  Source:  {COCODEM_SRC}")
    print(f"  Remote:  {REMOTE_BASE}  (replaces 111724.xyz + cfc.aroic.workers.dev)")
    print(f"  Local:   {CFC_BASE}  (replaces localhost:8787)")
    print("="*62)
